...
def fetch_critical_spares():
    critical_spares = []
    for row in sheet_tasks.iter_rows(min_row=2, max_row=sheet_tasks.max_row, values_only=True):
        if row[2] == 0 and is_spare_critical(row[1]):
            critical_spares.append({
                'spare_name': row[0],
                'spare_id': row[1],
            })
    return critical_spares
...
@app.route('/task_status_added', methods=['GET'])
def task_status_added():
    # Fetch and display only the newly added task in the Task Status page
    task_data = fetch_newly_added_task_data()
    return render_template('task_status.html', task_data=task_data)

def fetch_newly_added_task_data():
    # Fetch only the newly added task details from the main Excel file
    task_data = []
    for row in sheet_tasks.iter_rows(min_row=2, max_row=sheet_tasks.max_row, values_only=True):
        task_data.append({
            'spare_name': row[0],
    ...
    if request.method == 'POST':
        ...
        if spare_exists:
            ...
            if qty_requested <= available_qty:
                ...
                # Render the task_status.html template with the newly added task data
                return render_template('task_status.html', task_data=task_data)
            else:
                # Return an AJAX response with the available quantity
                return jsonify({'error': f"Available qty: {available_qty}"})
        else:
            return render_template('add_task.html', adjustment_msg="Spare not found.")

    return render_template('add_task.html')

def fetch_all_tasks_data():
    # Fetch all task details from the main Excel file
    task_data = []
    for row in sheet_tasks.iter_rows(min_row=2, values_only=True):
        task_data.append({
            'spare_name': row[0],
...
def complete_task(spare_id):
    ...
    # Subtract booking value from all stocks qty
    subtract_booking_from_all_stocks(booking)
...
def subtract_booking_from_all_stocks(booking):
    # Subtract booking value from all stocks qty
    for row in sheet_tasks.iter_rows(min_row=2, max_row=sheet_tasks.max_row):
        row[2].value = max(row[2].value - booking, 0)  # Ensure quantity doesn't go below zero
...
def task_status():
    ...
    for row in sheet_tasks.iter_rows(min_row=2, values_only=True):
        spare_name, spare_id, qty, booking = row

        # Only add rows where the booking value is not zero
        if booking != 0:
            task_data.append({
...
def history():
    ...
    for row in sheet_history.iter_rows(min_row=2, values_only=True):
        logging.debug("Row: %s", row)  # Log the row data
        if len(row) >= 5:  # Update to check for 5 elements in the row tuple
            history_data.append({
                'timestamp': row[0],
                'spare_name': row[1],
                'spare_id': row[2],
                'qty_requested': row[3],
                'purpose': row[4],  # Adjust index for 'purpose'
            })
    logging.debug("History Data: %s", history_data)  # Log the history data
    return render_template('history.html', history_data=history_data)

if __name__ == '__main__':
    app.run(debug=True)
#working OK1111111111111111111111111111111111111111111111111