...
def add_task(file_name, sheet_name, form_data):
    try:
        ...
        sheet.cell(row=row_num, column=5, value=form_data['purpose'])
    ...
    try:
        ...
        for row in range(2, all_stock_sheet.max_row + 1):
            if all_stock_sheet.cell(row=row, column=2).value == spare_id:
                # If spare ID exists, update the quantity or booking based on the presence of booking value
                if booking:
                    current_booking = all_stock_sheet.cell(row=row, column=4).value
                    all_stock_sheet.cell(row=row, column=4, value=current_booking + int(booking))
                else:
            ...
            if booking:
                new_row = (spare_name, spare_id, 0, booking, None)
            else:
                new_row = (spare_name, spare_id, int(qty), None, None)
            all_stock_sheet.append(new_row)

        wb.save(file_path)
        return True, 'Data updated successfully in All Stock.'
    except Exception as e:
        return False, str(e)
